...
export default function CharacterDatabase({
  ...
  function checkUnlockRequirements(requirements: Record<string, any>): boolean {
...
function InteractionsTab({ character }: { character: Character }) {
  ...
  const skillInteractionCharacter = {
    ...
    signatureSkills: Object.fromEntries(
      Object.entries(character.skills.signatureSkills).map(([key, skill]) => [
        key,
        {
          level: skill.level,
          experience: 0,
