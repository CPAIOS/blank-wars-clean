...
export default function TrainingGrounds({
  ...
  }, [currentCharacter]); // Depend on currentCharacter
  ...
  const availableActivities = useMemo(() => {
    ...
    return trainingActivities.filter(activity => {
      ...
      const meetsArchetype = !activity.requirements.archetype || 
        activity.requirements.archetype.includes(currentCharacter.archetype);
  ...
  const startTraining = async (activity: TrainingActivity) => {
    if (currentCharacter.energy < activity.energyCost) return;
    if (!canTrain()) return;
  ...
  const sendTrainingChatMessage = async () => {
    ...
    try {
      // Get available characters for context
      const availableCharacters = createDemoCharacterCollection();
      const currentCharacterContext = availableCharacters.find(c => c.name === currentCharacter.name) || availableCharacters[0];
      ...
      const trainingContext = {
        ...
        trainingEnvironment: {
          ...
          energyLevel: Math.round((currentCharacter.energy / currentCharacter.maxEnergy) * 100),
        ...
        recentTrainingEvents: [
          ...
          `Current energy: ${currentCharacter.energy}/${currentCharacter.maxEnergy}`,
...
>>>>>>> 48822e8 (Fix authentication persistence and character stat scaling)
          <motion.div 
            className="bg-gray-900/50 rounded-xl border border-gray-700 p-6"
