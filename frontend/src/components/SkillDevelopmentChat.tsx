...
const generateSkillAdvice = (character: EnhancedCharacter): string[] => {
  ...
  if (abilities) {
    const allAbilities = [...(abilities.active || []), ...(abilities.passive || []), ...(abilities.signature || [])];
    const attackAbilities = allAbilities.filter(a => a && typeof a === 'string' && a.toLowerCase().includes('attack'));
    const defenseAbilities = allAbilities.filter(a => a && typeof a === 'string' && (a.toLowerCase().includes('defense') || a.toLowerCase().includes('shield')));
    const specialAbilities = abilities.signature || [];
    const supportAbilities = allAbilities.filter(a => a && typeof a === 'string' && (a.toLowerCase().includes('heal') || a.toLowerCase().includes('support')));
