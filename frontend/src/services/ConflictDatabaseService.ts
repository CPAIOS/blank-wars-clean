...
class ConflictDatabaseService {
  ...
  private getFinancialContextData(characterId: string): {
    ...
    try {
      ...
      const character = this.characters.find(c => c.id === characterId);
      if (!character || !(character as any).financialData) {
      ...
      const { financialData } = character as any;
...
`;
  ...
  private setupFinancialEventListeners(): void {
    ...
    this.eventBus.subscribe('financial_crisis', (event: GameEvent) => {
      this.handleFinancialCrisisEvent(event);
    });

    // Listen for financial stress increase events
    this.eventBus.subscribe('financial_stress_increase', (event: GameEvent) => {
      this.handleFinancialStressEvent(event);
    });

    // Listen for financial spiral events
    this.eventBus.subscribe('financial_spiral_started', (event: GameEvent) => {
      this.handleFinancialSpiralEvent(event);
    });

    this.eventBus.subscribe('financial_spiral_deepening', (event: GameEvent) => {
      this.handleFinancialSpiralEvent(event);
    });
  }
   ...
   */
  private async handleFinancialCrisisEvent(event: GameEvent): Promise<void> {
    const { characterId, stressLevel, triggerReason } = event.metadata;
   ...
   */
  private async handleFinancialStressEvent(event: GameEvent): Promise<void> {
    const { characterId, stressLevel, triggerReason } = event.metadata;

    // Moderate probability of generating conflict during stress
    if (Math.random() < 0.4) {
      await this.generateFinancialConflict(characterId, 'medium', stressLevel, triggerReason);
    }
  }
   ...
   */
  private async handleFinancialSpiralEvent(event: GameEvent): Promise<void> {
    const { characterId, stressLevel, triggerReason } = event.metadata;

    // Very high probability of generating conflict during spiral
    if (Math.random() < 0.9) {
      await this.generateFinancialConflict(characterId, 'critical', stressLevel, triggerReason);
    }
  }
