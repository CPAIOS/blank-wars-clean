...
import { PostBattleAnalysisSystem, BattleMemory } from './postBattleAnalysis';
...
export class BattleEngine {
  ...
  static conductPreBattleHuddle(battleState: BattleState): PreBattleHuddle {
    ...
    huddle.huddleOutcome.aiJudgeComments = this.generateHuddleCommentary(
      { newChemistry: teamChemistryResult.overallChemistry, oldChemistry: teamChemistryResult.overallChemistry },
      characterReadiness.map(r => ({
        overallReadiness: (r.mentalReadiness + r.physicalReadiness + r.gameplanAdherence) / 3,
        characterName: r.characterId,
        mentalState: r.mentalReadiness,
        physicalCondition: r.physicalReadiness
      }))
    );
  ...
  static assessCharacterReadiness(battleChar: BattleCharacter, team: { characters: BattleCharacter[] }) {
    ...
    if (mental.strategyDeviationRisk > 70) predictedBehavior = 'independent';
    else if (mental.strategyDeviationRisk > 40) predictedBehavior = 'unpredictable';
  ...
  static generateCoachingOptions(team: { characters: BattleCharacter[] }, coachingData: any) {
    const options = [];
  ...
  static executeCharacterAction(
    ...
    if (gameplanAdherenceCheck.checkResult === 'follows_strategy' || gameplanAdherenceCheck.checkResult === 'slight_deviation') {
  ...
  static generateRogueAction(
    ...
    const rogueType = this.determineRogueActionType(character, gameplanCheck);
  ...
  static assessLeadershipDynamics(characters: BattleCharacter[]): string {
    ...
    const leaders = characters.filter(char =>
      char?.character?.archetype === 'scholar' ||
  ...
  private static determineRogueActionType(character: BattleCharacter, gameplanCheck: GameplanAdherenceCheck): string {
    ...
    if (gameplanCheck.checkResult === 'goes_rogue') return 'attacks_teammate';
  ...
  private static analyzePsychologyFactors(character: BattleCharacter, gameplanAdherenceCheck: GameplanAdherenceCheck): PsychologyFactor[] {
    ...
    if (character.mentalState.currentMentalHealth < 50) {
      factors.push({
        ...
        impact: 'negative',
        value: character.mentalState.currentMentalHealth,
    ...
    if (character.mentalState.confidence < 40) {
      factors.push({
        factor: 'confidence',
        impact: 'negative',
        value: character.mentalState.confidence,
    ...
    if (character.mentalState.teamTrust < 60) {
      factors.push({
        ...
        impact: 'negative',
        value: character.mentalState.teamTrust,
  ...
  private static identifyUrgentIssues(battleState: BattleState): UrgentIssue[] {
    ...
    battleState.teams.player.characters.forEach(char => {
      ...
      if (char.mentalState.currentMentalHealth < 20) {
        issues.push({
          ...
          immediateActions: ['Emergency timeout', 'Remove from active duty', 'Provide immediate counseling'],
          consequencesIfIgnored: ['Complete mental collapse', 'Risk to team safety', 'Long-term psychological damage']
        });
      }
      ...
      if (char.mentalState.stress > 90) {
        issues.push({
          ...
          immediateActions: ['Stress management techniques', 'Timeout for regrouping', 'Reassignment to support role'],
          consequencesIfIgnored: ['Panic attacks during battle', 'Erratic behavior', 'Performance degradation']
        });
      }
      ...
      if (char.mentalState.teamTrust < 20) {
        issues.push({
          ...
          immediateActions: ['Team mediation session', 'Individual coaching', 'Trust-building exercises'],
          consequencesIfIgnored: ['Team fragmentation', 'Sabotage of team efforts', 'Complete loss of cooperation']
        });
      }
      ...
      if (char.currentHealth < char.character.combatStats.maxHealth * 0.15) {
        issues.push({
          ...
          immediateActions: ['Medical timeout', 'Substitute player', 'Emergency medical care'],
          consequencesIfIgnored: ['Permanent injury', 'Team performance collapse', 'Medical emergency']
        });
      }
    });
    ...
    if (avgChemistry < 30) {
      issues.push({
        ...
        immediateActions: ['Emergency team meeting', 'Leadership intervention', 'Morale boosting speech'],
        consequencesIfIgnored: ['Complete team breakdown', 'Individual players giving up', 'Forfeit of battle']
      });
    }

    return issues;
  }
  private static assessTimeoutSeverity(issues: Array<{ severity: string }>): 'minor' | 'moderate' | 'critical' {
    const severities = issues.map(i => i.severity);
    if (severities.includes('critical')) return 'critical';
    if (severities.includes('urgent')) return 'moderate'; // Map 'urgent' to 'moderate'
    return 'minor'; // Map 'warning' to 'minor'
  }
    ...
    issues.forEach((issue, index) => {
      switch (issue.type) {
        case 'character_breakdown':
          actions.push({
            id: `emergency_counseling_${index}`,
            type: 'individual_coaching',
            description: `Provide immediate psychological support for character breakdown`,
            targetCharacters: ['affected_character'],
            effects: [
              {
                type: 'mental_health_restore',
                value: 30,
            ...
            requirements: ['coaching_skill_high'],
            successChance: 70,
            timeConsumed: 3
          });
          break;
        ...
        case 'team_conflict':
          actions.push({
            ...
            requirements: ['high_trust_with_team'],
            successChance: 60,
            timeConsumed: 4
          });
          break;
        ...
        case 'strategy_failing':
          actions.push({
            id: `strategy_adjustment_${index}`,
            type: 'strategic_pivot',
            description: `Adjust failing strategy and refocus team`,
            targetCharacters: ['all'],
            effects: [
              {
                ...
                value: 25,
                duration: 'rest_of_battle',
                description: 'Improve strategic execution and focus'
              }
            ],
        ...
        case 'morale_collapse':
          actions.push({
            id: `morale_boost_${index}`,
            type: 'team_rallying',
            description: `Emergency morale intervention to restore team spirit`,
            targetCharacters: ['all'],
            effects: [
              {
                type: 'mental_health_restore',
  ...
  private static assessTimeoutCharacterStates(battleState: BattleState): TimeoutCharacterState[] {
    return battleState.teams.player.characters.map(char => {
      ...
      const coachingNeeds: string[] = [];
      const quickFixOptions: QuickFix[] = [];
      const deepIssues: string[] = [];

      // Assess current issues
      if (char.mentalState.currentMentalHealth < 40) {
        currentIssues.push('Poor mental health');
        coachingNeeds.push('Immediate psychological support');
        quickFixOptions.push({
          action: 'Confidence boost',
          effect: 'Temporary mental health improvement',
          successChance: 70,
          riskLevel: 'low'
        });
      }
      ...
      if (char.mentalState.stress > 70) {
        currentIssues.push('High stress levels');
        coachingNeeds.push('Stress management techniques');
        quickFixOptions.push({
          action: 'Breathing exercises',
          effect: 'Reduce immediate stress',
          successChance: 85,
          riskLevel: 'low'
        });
      }

      if (char.mentalState.teamTrust < 50) {
        currentIssues.push('Low team trust');
        coachingNeeds.push('Trust building exercises');
        deepIssues.push('Fundamental trust issues with team');
      }

      if (char.battlePerformance.strategyDeviations > 3) {
        currentIssues.push('Poor gameplan adherence');
        coachingNeeds.push('Strategic refocusing');
        quickFixOptions.push({
          action: 'Strategy reminder',
          effect: 'Improve immediate focus',
          successChance: 75,
          riskLevel: 'low'
        });
      }

      if (char.currentHealth < char.character.combatStats.maxHealth * 0.3) {
        currentIssues.push('Severe injuries');
        coachingNeeds.push('Medical attention and pain management');
        deepIssues.push('Physical trauma requiring recovery time');
      }

      // Calculate receptiveness based on character state
      let receptiveness = 50; // Base receptiveness

      // Factors that increase receptiveness
      if (char.mentalState.teamTrust > 60) receptiveness += 20;
      if (char.mentalState.currentMentalHealth > 60) receptiveness += 15;

      // Factors that decrease receptiveness
      if (char.mentalState.stress > 80) receptiveness -= 25;
      if (char.battlePerformance.strategyDeviations > 4) receptiveness -= 20;
      if (char.character.personality.traits.includes('Stubborn')) receptiveness -= 15;
      if (char.character.personality.traits.includes('Prideful')) receptiveness -= 10;

      receptiveness = Math.max(0, Math.min(100, receptiveness));
  ...
  private static generateStrategicOptions(battleState: BattleState): StrategicOption[] {
    ...
    // Assess team condition to determine available strategic options
    const teamHealthPercentage = team.characters.reduce((sum, char) =>
      sum + (char.currentHealth / char.character.combatStats.maxHealth), 0) / team.characters.length;

    const teamMoraleLevel = team.currentMorale;
    const averageStress = team.characters.reduce((sum, char) => sum + char.mentalState.stress, 0) / team.characters.length;

    // Option 1: Aggressive Push (if team is healthy and confident)
    if (teamHealthPercentage > 0.7 && teamMoraleLevel > 60) {
      options.push({
        id: 'aggressive_push',
        name: 'Aggressive Push',
        description: 'Focus on overwhelming the opponent with coordinated attacks',
        pros: ['Higher damage output', 'Momentum building', 'Quick victory potential'],
        cons: ['Higher injury risk', 'Increased energy consumption', 'Vulnerable to counters'],
        characterReactions: team.characters.reduce((reactions, char) => {
          if (char.character.personality.traits.includes('Aggressive')) {
            reactions[char.character.id] = 'Enthusiastic - loves aggressive tactics';
          } else if (char.character.personality.traits.includes('Cautious')) {
            reactions[char.character.id] = 'Concerned - prefers safer approaches';
          } else {
            reactions[char.character.id] = 'Neutral - will follow the plan';
          }
          return reactions;
    ...
    // Option 2: Defensive Formation (if team is injured or stressed)
    if (teamHealthPercentage < 0.6 || averageStress > 60) {
      options.push({
        id: 'defensive_formation',
        name: 'Defensive Formation',
        description: 'Focus on protection and controlled counter-attacks',
        pros: ['Reduced injury risk', 'Energy conservation', 'Better positioning'],
        cons: ['Lower damage output', 'Longer battle duration', 'May lose momentum'],
        characterReactions: team.characters.reduce((reactions, char) => {
          if (char.character.personality.traits.includes('Cautious')) {
            reactions[char.character.id] = 'Relieved - feels safer with defensive tactics';
          } else if (char.character.personality.traits.includes('Impatient')) {
            reactions[char.character.id] = 'Frustrated - wants more action';
          } else {
            reactions[char.character.id] = 'Accepting - understands the necessity';
          }
          return reactions;
        }, {} as Record<string, string>),
    ...
    // Option 3: Focus Fire Strategy
    options.push({
      id: 'focus_fire',
      name: 'Focus Fire Strategy',
      description: 'Coordinate all attacks on single targets to eliminate them quickly',
      pros: ['Efficient target elimination', 'Clear team coordination', 'Psychological pressure on opponents'],
      cons: ['Predictable strategy', 'Vulnerable to area attacks', 'Requires precise timing'],
      characterReactions: team.characters.reduce((reactions, char) => {
        if (char.character.archetype === 'scholar') {
          reactions[char.character.id] = 'Approves - appreciates the tactical coordination';
        } else if (char.character.personality.traits.includes('Independent')) {
          reactions[char.character.id] = 'Reluctant - prefers choosing own targets';
        } else {
          reactions[char.character.id] = 'Cooperative - ready to follow coordination';
        }
        return reactions;
      }, {} as Record<string, string>),
    ...
    // Option 4: Adaptive Tactics (if team has mixed conditions)
    if (team.characters.some(char => char.mentalState.stress > 70) &&
        team.characters.some(char => char.mentalState.stress < 40)) {
      options.push({
        id: 'adaptive_tactics',
        name: 'Adaptive Tactics',
        description: 'Allow individual flexibility while maintaining team objectives',
        pros: ['Accommodates individual strengths', 'Reduces stress', 'Maintains team cohesion'],
        cons: ['Complex coordination', 'Potential for confusion', 'Harder to predict outcomes'],
        characterReactions: team.characters.reduce((reactions, char) => {
          if (char.mentalState.stress > 70) {
            reactions[char.character.id] = 'Grateful - appreciates the flexibility';
          } else if (char.character.personality.traits.includes('Disciplined')) {
            reactions[char.character.id] = 'Skeptical - prefers clear structure';
          } else {
            reactions[char.character.id] = 'Interested - curious about the approach';
          }
          return reactions;
        }, {} as Record<string, string>),
    ...
    // Option 5: Emergency Retreat (if team is in critical condition)
    if (teamHealthPercentage < 0.3 || teamMoraleLevel < 25) {
      options.push({
        id: 'emergency_retreat',
        name: 'Emergency Retreat',
        description: 'Immediate withdrawal to prevent further casualties',
        pros: ['Prevents total defeat', 'Preserves team for future battles', 'Minimizes injuries'],
        cons: ['Automatic loss', 'Severe morale impact', 'Long-term team confidence issues'],
        characterReactions: team.characters.reduce((reactions, char) => {
          if (char.character.personality.traits.includes('Brave')) {
            reactions[char.character.id] = 'Devastated - sees this as cowardice';
          } else if (char.currentHealth < char.character.combatStats.maxHealth * 0.2) {
            reactions[char.character.id] = 'Relieved - needs immediate medical care';
          } else {
            reactions[char.character.id] = 'Conflicted - understands but feels defeated';
          }
          return reactions;
        }, {} as Record<string, string>),
    ...
    battleState.teams.player.characters.forEach(char => {
      ...
      if (char.currentHealth < char.character.combatStats.maxHealth * 0.2) {
        consequences.push({
          ...
          longTermEffects: ['Reduced combat confidence', 'Increased anxiety in future battles'],
          recoveryTime: 5,
      ...
      if (char.battlePerformance.strategyDeviations > 5) {
        consequences.push({
          ...
          longTermEffects: ['Resistance to authority', 'Decreased team cooperation'],
          recoveryTime: 3,
      ...
      if (char.battlePerformance.successfulHits > 8 && char.battlePerformance.teamplayActions > 3) {
        consequences.push({
          ...
          longTermEffects: ['Increased self-confidence', 'Better performance under pressure'],
          recoveryTime: 0,
          treatmentOptions: []
        });
      }
    ...
    battleState.teams.player.characters.forEach(char => {
      ...
      if (hitRate < 0.5) {
        recommendations.push({
          ...
          expectedBenefit: 'Improved accuracy and precision in combat',
          timeRequired: 6
        });
      }
      ...
      if (char.battlePerformance.strategyDeviations > 2) {
        recommendations.push({
          ...
          expectedBenefit: 'Better adherence to team strategy and improved tactical awareness',
          timeRequired: 4
        });
      }
      ...
      if (char.mentalState.currentMentalHealth < 50) {
        recommendations.push({
          ...
          expectedBenefit: 'Improved mental health and stress management abilities',
          timeRequired: 7
        });
      }
      ...
      if (char.battlePerformance.teamplayActions < 2) {
        recommendations.push({
          ...
          expectedBenefit: 'Improved team coordination and trust building',
          timeRequired: 4
        });
      }
    });
  ...
  private static analyzeChemistryEvolution(battleState: BattleState): ChemistryEvolution {
    ...
    const result: ChemistryEvolution = {
      oldChemistry,
      newChemistry,
      evolutionFactors,
      ...
      cultureShift: newChemistry > oldChemistry + 10 ? 'More cohesive and trusting' :
                   newChemistry < oldChemistry - 10 ? 'More fractured and tense' :
